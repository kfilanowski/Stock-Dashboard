...
class PriceHistoryService:
    ...
    def get_latest_timestamps_batch(
        self, 
        tickers: List[str], 
        interval: str
    ) -> Dict[str, Optional[datetime]]:
        """
        Get the latest timestamps for multiple tickers in one efficient query.
        
        This is used for incremental batch fetching - we find the oldest
        "since" timestamp and fetch from that point for all tickers.
        ...
        Returns:
            Dict mapping ticker -> latest timestamp (or None if no data).
        """
        ...
        with Session(self._engine) as session:
            ...
            result = session.execute(
                select(
                    IntradayPriceHistory.ticker,
                    func.max(IntradayPriceHistory.timestamp).label('latest')
                )
                .where(IntradayPriceHistory.ticker.in_(tickers))
                .where(IntradayPriceHistory.interval == interval)
                .group_by(IntradayPriceHistory.ticker)
            )
            
            # Build result dict, defaulting to None for tickers with no data
            timestamps = {ticker: None for ticker in tickers}
            for row in result:
                timestamps[row.ticker] = row.latest
            
            return timestamps
    
    def get_intraday_since(
        self, 
        ticker: str, 
        interval: str, 
        since: datetime
        ...
        """
        Get intraday data since a specific timestamp (exclusive).
        
        This is the core method for incremental updates - only fetch
        data points newer than what we already have.
        
        Args:
            ticker: Stock ticker symbol.
            interval: Time interval.
            since: Fetch data after this timestamp (exclusive).
            
        Returns:
            List of new price data points.
        """
        ticker = ticker.upper()
        ...
        with Session(self._engine) as session:
            result = session.execute(
                ...
                .where(IntradayPriceHistory.ticker == ticker)
                .where(IntradayPriceHistory.interval == interval)
                .where(IntradayPriceHistory.timestamp > since)
                .order_by(IntradayPriceHistory.timestamp)
            )
            rows = result.scalars().all()
