...
def calculate_all_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Calculate each indicator and merge
    result = pd.concat([result, calculate_rsi(df)], axis=1)
    result = pd.concat([result, calculate_macd(df)], axis=1)
    result = pd.concat([result, calculate_bollinger_bands(df)], axis=1)
    result = pd.concat([result, calculate_bollinger_squeeze(df)], axis=1)
    result = pd.concat([result, calculate_adx(df)], axis=1)
...
def calculate_indicator_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # RSI Signal (mean reversion interpretation)
    rsi = indicators['rsi_value']
    adx_trending = indicators['adx_trending']
    
    # Base RSI signal (mean reversion)
    base_rsi = np.where(rsi <= 30, 1.0 - (rsi / 30) * 0.5,
               np.where(rsi <= 50, 0.5 - ((rsi - 30) / 20) * 0.5,
               np.where(rsi <= 70, -((rsi - 50) / 20) * 0.5,
                        -0.5 - ((rsi - 70) / 30) * 0.5)))
    
    # Regime override: Trending market (ADX > 25)
    trending_bullish = (rsi >= 65) & (rsi < 85) & adx_trending
    ...
    adx_signal = np.where(adx < 20, 0.0,  # No trend
                 np.where(adx < 25, 0.3,
                 np.where(adx < 40, 0.6, 0.9)))
    ...
    momentum_signal = np.where(momentum_short > 10, 1.0,
                      np.where(momentum_short > 5, 0.6,
                      np.where(momentum_short > 0, 0.2,
                      np.where(momentum_short > -5, -0.2,
                      np.where(momentum_short > -10, -0.6, -1.0)))))
    signals['momentum_signal'] = np.clip(momentum_signal, -1, 1).round(4)
    
    # Volume Signal (direction-aware like TypeScript)
    ...
    # Volume magnitude
    vol_magnitude = np.where(volume_ratio > 2.0, 1.0,
                    np.where(volume_ratio > 1.5, 0.6,
                    np.where(volume_ratio > 1.0, 0.3, 0.0)))
    ...
    # Price Position Signal (trend-aware interpretation)
    ...
    # Use trend signal when trending, mean-reversion when ranging
